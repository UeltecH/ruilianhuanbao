	...
	<div class="jiance">
		...
		<div class="inp_list">
			...
			<div class="diqu" @click='diqu'>
				<span>地区：</span>
				<input type="text" readonly="readonly" v-model="diqu_text" placeholder="选择省/市/区">
			</div>
			<div class="dizhi">
				<span>地址：</span>
			...
			<div class="caiyangdian">
				<span>面积：</span>
				<input type="number" placeholder="请输入面积" v-model="caiyangdian">
				<span>（m²）</span>
			</div>
			<div class="zongjine">
				<span>总金额：</span>
...
<script>
	...
	export default {
		...
		mounted:function(){
			...
			this.addr[0].values = sheng
			this.sheng = sheng
			this.sheng_count = sheng[0]
			let shi = []
			for(let j in city[sheng[0]]){
				shi.push(j)
			}
			this.shi = shi
			this.addr[2].values = shi
			this.shi_count =shi[0]
			let qu = []
			for(let k in city[sheng[0]][shi[0]]){
				qu.push(city[sheng[0]][shi[0]][k])
			}
			this.addr[4].values = qu
			this.qu = qu
			this.qu_count = qu[0]
		}
	}
